...
bool IndexFloat::ProcessingFrame(char *data, int count, QVector<float> &dd)
{
    if (count <= 0)
        return false;
    ...
    if (count % 4 == 0) {
        ...
        int data_count = count /4 - 2; // 除去帧头与帧尾
        int start_index = 0;
        for (int i = 0; i < count - 4; i += 4) {
            ...
            if (i == 0) {
                start_index = value;
                while (buf_.count() < (start_index + data_count)) {
                    buf_.append(0);
                }
            } else {
                buf_[start_index + i/4 - 1] = value;
            }
        }

        for (int i = 0; i < buf_.count(); i++) {
            dd.append(buf_.at(i));
        }
        return true;
    }
...
void IndexFloat::ProcessingDatas(char *data, int count)
{
