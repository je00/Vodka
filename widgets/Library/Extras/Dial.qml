...
//    style: Settings.styleComponent(Settings.style, "DialStyle.qml", dial)
    ...
    */
    ...
    MyMouseArea {
        ...
        onPositionChanged: {
            mouse_range.value = valueFromPoint(mouseX, mouseY);
            if (pressed) {
                var targetValue = valueFromPoint(mouseX, mouseY);
                if (value === range.minimumValue &&
                        targetValue > (range.minimumValue + (range.maximumValue - range.minimumValue)*1/3)
                        || value === range.maximumValue &&
                        targetValue < (range.minimumValue + (range.maximumValue - range.minimumValue)*2/3))
                    return;
                value = targetValue;
            }
        }
