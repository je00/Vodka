...
ResizableRectangle {
    ...
    Item {
        ...
        property color colorLight_: {
            if (colorLightFollowCh) {
                if (ch_menu.bind_obj)
                    ch_menu.bind_obj.color
                else
                    appTheme.goodColor
            } else if (colorLightFollow) {
                appTheme.goodColor
            } else
                colorLight
        }
    ...
    DelayButton {
        ...
        style: DelayButtonStyle {
            ...
            Gradient {
                id: __progressBarGradient
                GradientStop {
                    position: 0
                    color: Qt.lighter(theme.colorLight_, 1.2)
                }
                GradientStop {
                    position: 1
            ...
            on__AppThemeChanged: {
                __progressBarGradient.stops[0].color = Qt.lighter(theme.colorLight_, 1.2);
                __progressBarGradient.stops[1].color = theme.colorLight_;
    ...
    MyIconMenu {
        ...
        SettingsMenu {
            ...
            MyMenuItem {
                text_center: true
                text: qsTr("延时(ms)：")
                value_text: root.delay
                value_editable: true
                plus_minus_on: true
                onPlus_triggered: root.delay += 500;
                onMinus_triggered: root.delay = Math.max(0, root.delay - 500);
                onValue_inputed: {
                    var tmp = parseInt(text);
                    if (isNaN(tmp))
                        tmp = 1000;
                    root.delay = Math.max(0, tmp);
                }
            }
            MyMenuSeparator {
            }
